...
category: Buttons
context: form
summary: The UpdateLink tag renders as a hyperlink that, when clicked, executes the SubmitCommand of the enclosing EditForm.
keywords:
  - update
  - link
  - form
...
The UpdateLink tag renders as a hyperlink at run-time. When clicked, it executes the `<SubmitCommand>` associated with the enclosing `<EditForm>` — same behavior as [`<UpdateButton>`](update-button.md), just rendered as an inline `<a>` element instead of a push-button.

::: info Use only inside EditForm
The UpdateLink triggers the `<EditForm>`'s `<SubmitCommand>`. It has no effect outside an `<EditForm>`.
:::
...
<EditForm>
  ...
  <table>
    ...
    <tr>
      <td colspan="2"><UpdateLink Text="Update"/> <CancelLink Text="Cancel"/></td>
    </tr>
  </table>
