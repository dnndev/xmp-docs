---
id: form-calendar-link
title: CalendarLink
category: Buttons
context: form
summary: The CalendarLink tag renders as a hyperlink that opens a date-picker calendar and writes the selected date to a target text box.
keywords:
  - calendar
  - link
  - form
...
The CalendarLink tag renders a hyperlink that opens a pop-up date-picker. When the user picks a date, it's written into the [target](#prop-target) control (typically a `<TextBox>`) — same behavior as [`<CalendarButton>`](calendar-button.md), just rendered as an inline `<a>` element.

::: info Validation is skipped
CalendarLink sets `CausesValidation="False"` automatically, so opening the calendar won't trigger client-side validation on the form.
:::
