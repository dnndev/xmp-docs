...
category: Buttons
context: form
summary: The UpdateImage tag renders as a clickable image that, when clicked, executes the SubmitCommand of the enclosing EditForm.
keywords:
  - update
  - image
  - form
...
The UpdateImage tag renders as a clickable image at run-time. When clicked, it executes the `<SubmitCommand>` associated with the enclosing `<EditForm>` — same behavior as [`<UpdateButton>`](update-button.md), just rendered as an image instead of a push-button.

::: info Use only inside EditForm
The UpdateImage triggers the `<EditForm>`'s `<SubmitCommand>`. It has no effect outside an `<EditForm>`.
:::
...
*   <span id="prop-imageurl">**ImageUrl**</span>: Path to the image file. The tilde (`~`) represents the application root, so `ImageUrl="~/images/update.gif"` resolves to `/dnntestsite/images/update.gif` on a development install and `/images/update.gif` on production. Use a portal-relative path (e.g. `/Portals/0/images/update.gif`) for portal-specific images.
