...
category: Buttons
context: form
summary: The CalendarImage tag renders as a clickable image that opens a date-picker calendar and writes the selected date to a target text box.
keywords:
  - calendar
  - image
  - form
...
The CalendarImage tag renders a clickable image that opens a pop-up date-picker. When the user picks a date, it's written into the [target](#prop-target) control (typically a `<TextBox>`) — same behavior as [`<CalendarButton>`](calendar-button.md), just rendered as an image (a small calendar icon is the usual choice).
