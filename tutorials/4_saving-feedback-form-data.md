...
summary: >-
  ...
  use? That's what we'll be covering in this tutorial.
keywords:
  - save
  - form
  - data
...
5.  From this starting point, we'll need to tell XMod Pro what command to execute when the user clicks the `<addbutton>` control. This is done via the `<SubmitCommand>` tag. This tag defines the command that is executed when the user submits the form.
...
6.  **Validation**: Whenever you allow users to enter data you should validate that input. Since we're focused on the mechanics of linking your form to a data source, we'll touch on validation, but you'll probably want to add more.
    ...
    In line 2 above, we've added a second validator to the Email text box. It is a required field validator. This will not allow the form to be submitted if the Email textbox is empty. The second validator (line 3) is the Email Validator we added in Tutorial 3. This checks to see if the email address is in a valid format (somename@somesite.com).

7.  That's all the changes we need to make. Our modified form definition looks like this:
...
8.  Once you've made the changes, click **Save** in the Code Editor.
9.  The form will look exactly the same as before, except now it will add the data to the database as well as send the email:
    ![](./Walkthrough3_FormView.png)
10. When the user clicks the "Send Your Question" button, assuming the email address is correct, an email will be sent to "you@yoursite.com" - or whatever address you specified in the `<email>` tag.
