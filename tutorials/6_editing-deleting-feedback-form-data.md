...
summary: >-
  ...
  view to display a list of the feedback we've received from customers and
  provided a button to add feedback. Now we'll modify that view and the
  feedback form to enable us to edit and delete feedback.
keywords:
  - modify
  - form
  - data
...
In the last tutorial, we created a view to display a list of the feedback we've received from customers and provided a button to add feedback. Now we'll modify that view and the feedback form to enable us to edit and delete feedback.
...
4.  The current view is displayed below. The lines we'll be modifying are highlighted.
    ...
    **Edit Roles**
    ...
    This property of the `<xmod:template>` tag is optional. Hosts and Administrators can always edit records. If you want members of other roles to have permission, you'll need to specify it using the EditRoles property. In this case, members of "Role 1" or "Role 3" can edit records as well. Note that you should replace these with appropriate role names for your site.
...
5.  Click **Save** in the Code Editor. Your display should now look something like this:
...
7.  Since we won't be modifying the `<AddForm>` code, we'll omit that for the sake of brevity. After the closing `</AddForm>` tag, add the following highlighted code:
    ...
    **No Email**
    ...
    One other difference between the Add and Edit form is the lack of an `<email>` tag in the Edit form. For this solution, we only want to be notified when a user has created a new feedback record, not when that record has been edited. Should you need a notification sent when a record is updated, you can add an `<email>` tag, following the example in the `<AddForm>`.
...
8.  Once you've made the changes to your form, click **Save** in the Code Editor.
    ![](./Walkthrough6_DisplayWithEditButton.png)

9.  Now you can click the Edit button: the pencil graphic in the example above. You will be presented with the Edit Form, with the controls pre-populated with values from the record.
    ![](./Walkthrough6_EditForm_BeforeChange.png)

10. You can make some changes to the record. In the example below, we've changed the name to "Harry Truman":
...
13. Make the following changes — highlighted below:
    ...
    **Delete Roles**
    ...
    This property of the `<xmod:template>` tag is optional. When used, you specify a semi-colon-delimited list of DotNetNuke security role names. Only members of those roles will see the delete button. Hosts and Administrators can always delete records.

    **Delete Command**

    In order to delete a record, you'll need to specify a `<DeleteCommand>` tag in your view. As with the `<DetailDataSource>` tag, your command should delete only one record. Usually this is done by specifying a parameter or set of parameters that limit what record(s) are deleted. In this case, we use the FeedbackId value to accomplish that task. So, the tag includes a `<parameter>` tag with the name "FeedbackId".
...
14. Click **Save** in the Code Editor. Your display should now look similar to this (remember we changed the Name to "Harry Truman" earlier):
    ![](./Walkthrough6_DisplayWithDeleteButton.png)

15. Now, let's add a new record and delete it. First, click the Submit Feedback button and add a new record.
...
16. Click the red "X" (or whatever image you used) next to the new record you've added.
