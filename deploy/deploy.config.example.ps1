# deploy.config.example.ps1  (v5 docs)
#
...
@{
    ...
    # Absolute remote path to THIS version's folder. MUST end in /v5/ — the deploy
    # script refuses to run otherwise, so a wrong value can't mirror-delete the
    # wrong folder.
    RemoteVersionPath = '/httpdocs/help/xmodpro/v5/'

    # Absolute remote path to the shared /help/xmodpro/ root. Only used by -Root
    # (uploads the redirect web.config). Leave as-is if you don't use -Root.
    RemoteRootPath = '/httpdocs/help/xmodpro/'
