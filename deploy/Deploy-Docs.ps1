...
<#
.SYNOPSIS
    Build and deploy the XMod Pro v5 (VitePress) docs to production via WinSCP.
.DESCRIPTION
    Builds the VitePress site and mirrors .vitepress/dist to the production
    /help/xmodpro/v5/ folder using WinSCP `synchronize -delete`. Connection
...
.PARAMETER SkipBuild
    Deploy the existing .vitepress/dist without rebuilding.
.PARAMETER DryRun
    Preview the synchronize (WinSCP -preview); apply nothing.
.PARAMETER Verify
    After deploy, run HTTP checks against the live URLs.
.PARAMETER Root
    Instead of deploying the site, upload deploy/help-xmodpro-root-web.config to
    the shared /help/xmodpro/ root as web.config (plain put, no delete).
...
# This script deploys ONLY the v5 version folder. The v4 repo has its own copy.
$ExpectedVersionSegment = 'v5'
$DistSubPath = '.vitepress\dist'
...
if ($Root) {
    ...
    $put  = "put `"$webConfig`" `"$($cfg.RemoteRootPath)web.config`""
    Write-Host "Uploading root web.config to $($cfg.RemoteRootPath)..." -ForegroundColor Cyan
    Invoke-WinScp -WinScp $winscp -Commands @('option batch abort','option confirm off',$open,$put,'exit')
    Write-Host 'Root web.config uploaded.' -ForegroundColor Green
    if ($Verify) { Test-Deployment }
    return
}

# Version-folder deploy
